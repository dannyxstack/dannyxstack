...
### Planet Edge Ltd - Go 后端工程师
📍 Perth | 🗓 2024.10 – Now
- 负责设计与实现一个面向社交媒体运营的 AI 自动化任务流平台，支持从内容生成 → 视频渲染 → 自动发布的端到端流程。
- 使用 Golang 开发核心服务，基于 REST/gRPC API 提供任务提交与查询，所有任务采用异步化设计，避免长连接阻塞。
- 搭建了轻量级 任务编排引擎（Temporal, Golang state machine + Redis 队列），实现任务的并行调度、状态追踪与幂等重试，支持文案生成、竞品分析、视频渲染与社媒发布等 Worker。
- 数据层采用 Postgres + Redis + S3：Postgres 管理用户与任务元数据，Redis 用于队列与缓存，S3 存储生成的媒体文件。
- 在 AWS Fargate 上部署服务，结合 RDS(Postgres)、ElastiCache(Redis) 与 S3，降低运维复杂度；通过 CloudWatch + Prometheus 监控任务成功率、队列长度与失败重试情况。
- 成功支撑了小规模真实用户场景，实现了 自动生成教程文案 → 渲染短视频 → 发布到目标社媒账号 的闭环流程，证明了平台的可行性。
